...
export default function AchievementsPage() {
  ...
  const favEntry = (Object.entries(stats.gamesPerType) as [GameKey, number][])
    .sort(([, a], [, b]) => b - a)[0];
  const favorite = favEntry ? howTo[favEntry[0]].label : "—";
  const medalsCount = achievements.filter((a) => a.unlocked).length;

  return (
    <div className="mx-auto w-full max-w-4xl px-4 py-8">
      <h1 className="text-3xl font-black md:text-4xl">{t("ach_title")}</h1>
      <p className="mt-2 text-sm text-gray-400">{t("ach_subtitle")}</p>

      {/* Stats overview */}
      <section className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-5">
        <Stat label={t("home_total_games")} value={stats.totalGames} />
        <Stat label={t("ach_favourite")} value={favorite} />
        <Stat label={t("home_best_streak")} value={`${stats.bestStreak} ${t("home_days_short")}`} />
        <Stat label={t("ach_time_played")} value={formatTime(stats.totalSeconds)} />
        <Stat label={t("home_medals")} value={medalsCount} />
      </section>

      {/* Streak calendar */}
      <section className="mt-8 rounded-2xl border border-[#2a2a2a] bg-[#1a1a1a] p-4">
        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-400">{t("ach_last_90")}</h2>
        ...
        <p className="mt-3 text-xs text-gray-500">
          {t("ach_current_streak")} <span className="font-bold text-emerald-300">{stats.streakDays}</span> {t("ach_days")} · {t("ach_best")}{" "}
      ...
      <section className="mt-8">
        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-400">{t("home_medals")}</h2>
